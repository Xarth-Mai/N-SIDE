...
先读[美术](../../../docs/production/art.md)与本轮修改边界。参考用于提取表现方法，正式人物、地点和既定空间仍由本项目设计决定
...
5. **把制作接回既有流程**：资产用 `create-game-assets` 和[资产管理](../../../docs/production/assets.md)，工程以 `game/Cargo.lock` 及真实 Bevy 入口实现。视频、3D 生成与付费服务只有在当前环境确实可用且授权时才安排；普通模型可沿现有几何和 GLB 管线制作，不强制 image-to-3D
6. **回看实现结果**：按[运行验收](../../../docs/production/runtime-validation.md)使用同场景、近似镜头和已记录输入比较。机器断言检查状态，连续帧检查方向、遮挡、动作与可读性。已读过设计的作者或 Codex 评图标为 `self-audit`，不当作盲测或陌生玩家反馈
