...
const signedArea=p=>edges(p).reduce((n,[a,b])=>n+a[0]*b[1]-b[0]*a[1],0)/2
export const polygonArea=p=>Math.abs(signedArea(p))
export const onBoundary=(p,polygon)=>edges(polygon).some(([a,b])=>Math.abs(cross(a,b,p))<EPS&&p[0]>=Math.min(a[0],b[0])-EPS&&p[0]<=Math.max(a[0],b[0])+EPS&&p[1]>=Math.min(a[1],b[1])-EPS&&p[1]<=Math.max(a[1],b[1])+EPS)
export function pointInside(p,polygon,boundary=true){
...
export function segmentIntervals(a,b,polygon,boundary=false){
...
export const segmentInside=(a,b,p)=>pointInside(a,p)&&pointInside(b,p)&&Math.abs(segmentIntervals(a,b,p,true).reduce((n,[lo,hi])=>n+hi-lo,0)-1)<EPS
export const polygonInside=(p,container)=>edges(p).every(([a,b])=>segmentInside(a,b,container))
...
export const roadWidth=r=>r.width??({main:10,avenue:15,shore:7,bridge:16,deck:5,steps:6,trail:4,service:5,landing:9}[r.kind]??7)
export const roadAllowed=(road,user='public')=>road.access==='controlled'?(road.users??[]).includes(user):['service','resident'].includes(road.access)?road.access===user:!road.access||road.access==='public'

export function roadHitsBuilding(a,b,width,building){
