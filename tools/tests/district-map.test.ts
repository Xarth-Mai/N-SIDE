...
const linked=(a: string,b: string,user='public')=>data.roads.some(r=>roadAllowed(r,user)&&r.nodes.some((id,i)=>i>0&&((id===a&&r.nodes[i-1]===b)||(id===b&&r.nodes[i-1]===a))))
const reachable=(user: string,start='station')=>{
...
test('orthographic projection preserves parallel edges and separates elevation from depth', async () => {
  const {project,depth}=await import('../district-map.ts')
  const a=project([10,20,0]),b=project([10,20,10])
  assert.equal(a[0],b[0])
  assert.ok(b[1]<a[1])
  const delta=(a: Point,b: Point)=>a.map((v,i)=>v-b[i])
...
test('scene keeps platforms below volumes and geometry is finite', async () => {
  const {buildScene}=await import('../district-map.ts'),scene=buildScene(data)
...
test('expanded plan counts destinations separately and keeps parcel attachments valid', async () => {
  const {inside,planStats,parcelStats}=await import('../district-plan.ts')
...
test('sections follow connected routes and distinguish a lift from a slope', async () => {
  ...
  const linked=(a: string,b: string)=>data.roads.some(r=>r.nodes.some((n,i)=>i>0&&((n===a&&r.nodes[i-1]===b)||(n===b&&r.nodes[i-1]===a))))
...
test('geometry handles concave boundaries, shared walls, road width and local elevation', () => {
  ...
  const turn=(p: Point[])=>p.map(([x,y])=>[(x-y)/Math.sqrt(2)+17,(x+y)/Math.sqrt(2)-9])
...
test('district ground-road crossings share a junction and its elevation', () => {
  const issues=[]
  const cross=(a: Point,b: Point)=>a[0]*b[1]-a[1]*b[0],subtract=(a: Point,b: Point)=>a.map((n,i)=>n-b[i])
...
test('cinema roof and exterior platform have distinct footprints and supported paths', async () => {
  ...
  for(const r of data.roads.filter(r=>r.surface)) {
    ...
    assert.ok(surface,r.surface ?? "surface")
...
test('housing estimates use residential floor area rather than the volume count', async () => {
  const {housingEstimate}=await import('../district-plan.ts')
  const estimate=housingEstimate({parcels:[{polygon:[[0,0],[10,0],[10,20],[0,20]],housing:{coverage:.5,floors:4,share:.75}},{polygon:[[0,0],[100,0],[100,100],[0,100]]}],housingAssumptions:{netRatio:.8,unitArea:60,occupancy:[.5,1],household:[3,3]}} as District)
  assert.equal(estimate.gross,300)
  assert.equal(estimate.units,4)
  assert.deepEqual(estimate.residents,[6,12])
  const drawn=housingEstimate({parcels:[{id:'P',housing:{share:.5}}],buildings:[{bank:'district',parcel:'P',polygon:[[0,0],[10,0],[10,10],[0,10]],elevation:0,height:6,design:{floors:[{z:0},{z:3},{z:6}]}}],housingAssumptions:{netRatio:.8,unitArea:40,occupancy:[1,1],household:[2,2]}} as District,true)
...
test('station mixed-use and interest samples connect customer and backstage rooms through their own entrances', () => {
  ...
  for(const building of data.buildings.filter(b=>segment!.buildings.includes(b.id)&&['mixed','interest','maker'].includes(b.design!.type)&&b.design!.floors[0].rooms)){
    ...
    for(const [role,kind] of [['public','shop'],['service','service']]){
      ...
      const allowed: typeof rooms=rooms.filter(r=>r.kind===kind||r.kind==='core')
      const reached: Set<typeof rooms[number]>=new Set(allowed.filter(r=>entries.some(e=>onBoundary(data.nodes[e.node],r.polygon))))
      ...
      const destinations: typeof rooms=rooms.filter(r=>r.kind===kind)
...
test('shared stairs and separate apartments connect through shared circulation', () => {
  for(const [id,index] of ([['V-13',0],['V-57',1]] as [string,number][])) {
