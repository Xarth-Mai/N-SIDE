...
import { readTasks, renderBoard, run } from '../tasks.mjs'
...
function fixture() {
  ...
  const body = '# 实验\n\n## 目标与范围\n\n验证假设\n\n## 验收条件\n\n保留真实否定结果\n\n## 当前工作与下一步\n\n按结果调整规格\n\n## 结果与证据\n\n假设已被否定，实验完成\n'
  const card = (value = base, path = `todo/tasks/${value.id}-experiment.md`) => write(path, `---\n${Bun.YAML.stringify(value)}\n---\n${body}`)
...
const failure = (f, part) => { const result = readTasks(f.root); expect(result.ok).toBe(false); expect(result.issues.some(issue => issue.message.includes(part))).toBe(true) }
