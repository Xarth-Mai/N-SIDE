...
Bun 工具链与文档图片规则由仓库根目录 `AGENTS.md` 的“资源与工具链”维护，Wiki 命令见[制作说明](production/wiki.md)
...
Wiki 分类入口使用 `index.md`。正文采用标准 Markdown，页面链接使用相对 `.md` 路径。仓库内文档图片统一使用以质量参数 `80` 编码的 WebP，Wiki 图片以 `/images/文件名.webp` 引用，文件保存在 `docs/public/images/`。模板占位符使用行内代码或代码块

VitePress 配置集中在 `.vitepress/`。侧栏从主题目录与页面标题生成。JSON、CSV 等结构资料维护在各自内容目录，发布时随构建导出
...
Git 保存修改历史。当前正文随设计与实现同步更新，重要改动在任务记录中说明原因与影响
