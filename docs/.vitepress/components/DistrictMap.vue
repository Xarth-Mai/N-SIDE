...
function choose(id){
  selected.value=id
  if(camera.value.width>600){const [x,y]=project(data.places.find(p=>p.id===id).position);camera.value={x,y,width:360};frame.value='detail'}
}
function zoom(factor){frame.value='detail';camera.value={...camera.value,width:Math.max(220,Math.min(2400,camera.value.width*factor))}}
let pointer=null
function down(e){if(e.button!==0||e.target.closest('[data-place]'))return;pointer={id:e.pointerId,x:e.clientX,y:e.clientY};svg.value.setPointerCapture(e.pointerId)}
function move(e){if(pointer?.id!==e.pointerId)return;const k=scale.value;camera.value={...camera.value,x:camera.value.x-(e.clientX-pointer.x)/k,y:camera.value.y-(e.clientY-pointer.y)/k};frame.value='detail';pointer={id:e.pointerId,x:e.clientX,y:e.clientY}}
function up(e){if(pointer?.id!==e.pointerId)return;pointer=null;if(svg.value.hasPointerCapture(e.pointerId))svg.value.releasePointerCapture(e.pointerId)}
function key(e){
...
<template>
  <section class="district-map" :class="{expanded}" aria-label="N街区导览图" @keydown.esc="expanded=false">
    ...
    <div class="map-stage">
      <svg ref="svg" class="map-canvas" :viewBox="view" tabindex="0" role="group" aria-label="街区斜俯视导览，方向键移动，加减键缩放；地点也可从下方列表选择" @keydown="key" @pointerdown="down" @pointermove="move" @pointerup="up" @pointercancel="up" @lostpointercapture="pointer=null">
...
.expanded{position:fixed;inset:0;z-index:1000;margin:0;border:0;border-radius:0;overflow:auto;height:100dvh}.expanded .map-canvas{height:calc(100dvh - 160px);min-height:320px}.expanded .map-detail{position:sticky;bottom:0;z-index:2;box-shadow:0 -4px 18px #485b4320}
@media(max-width:600px){.map-header{padding:14px 12px 8px}.map-header h3{font-size:18px!important}.map-toolbar{padding:0 10px 10px;gap:4px}.map-tabs button{padding:6px 8px}.map-label-toggle{margin-left:4px}.map-canvas{height:560px}.map-index>div{grid-template-columns:repeat(2,minmax(0,1fr))}.map-header button{padding:6px 9px}.expanded .map-canvas{touch-action:none}}
</style>
